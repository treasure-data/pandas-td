...
class Session(object):
    ...
    def __repr__(self):
        return "<{0}.{1} index={2} name={3} status={4}>".format(
            self.__module__, self.__class__.__name__, self.index, self.name, self.status
        )
...
class JobQueue(object):
    def __init__(self, name=None, workers=4):
        ...
        self.download_pool = concurrent.futures.ThreadPoolExecutor(
        ...
        if len(self.notifiers) == 0:
            ...
            self.notifiers.append(
                self.get_notifier("pandas_td.notifier.LoggingNotifier")
            )
    ...
    def submit_job(self, job_id, con, name=None, callback=None):
        ...
        session.download_future = self.download_pool.submit(
            self.do_download, session, job
        )
    ...
    def notification_callback(self, future):
        ...
        if session.status in ["error", "killed"]:
            ...
            self.post_message(
                "error", "Query failed.  Run `{0}` for details.".format(command)
            )
            session.notified = True
        ...
        notification_list = [
            session for session in self.sessions if not session.notified
        ]
