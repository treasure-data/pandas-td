...
class SlackNotifier(BaseNotifier):
    ...
    def post(self, message=None, attachment=None, notify=False):
        ...
        params = {"username": self.username, "icon_url": self.icon_url}
        if message:
            params["text"] = message
            params["parse"] = "full"
        if attachment:
    ...
    def post_session(self, session, notify=False):
        ...
        if self.author:
            digest = hashlib.md5(
                self.author.strip().lower().encode("ascii")
            ).hexdigest()
        ...
        if session.job_start_at:
            ...
            fields.append(
                {"title": "Duration", "value": str(job_duration), "short": True}
            )
        # download
        if session.download_start_at and session.download_end_at:
            ...
            fields.append(
                {"title": "Duration", "value": str(download_duration), "short": True}
            )
        # fields
